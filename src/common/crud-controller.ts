...
export default class CrudController<T> {
  ...
  public getByDestination = async (
    ...
    try {
      ...
      const destination = encodeURIComponent(request.params[this.destination]);

      const entity: T = await this.crudResolver.getAllByName(destination);
